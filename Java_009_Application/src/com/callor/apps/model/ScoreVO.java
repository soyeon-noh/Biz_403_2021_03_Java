...
public class ScoreVO { // 7개의 변수가 선언된 객체만들기
	//ScoreEx_01
	...
	public int num; // VO에있는 변수를 모두 public으로 만드는 것은 위험하다.
	public String name; //main() 메서드에서 실수로 변경당할 위험이 있기 때문.
	public int kor;
	public int eng;
	public int math;
	 ...
	 * 
	 * kor, eng, math는 어디에서든지 저장, 읽기가 가능한 상태이어야 하므로
	 * public으로 선언을 한다.
	 * 
	 * 그런데 
	 ...
	 * 평균값이 엉뚱하게 계산되는 결과가 나타난다.
	 * 이런경우 total, avg 변수를 임의로 수정(저장)하지 못하도록
	 * 막는 장치가 필요하다.
	 */
	/*
	 * VO 클래스의 멤버변수를 private으로 선언하면
	 * 다른곳에서 객체를 통해(점연결 연산자를 통해) 
	 * 직접 접근이 불가능해진다.
	 * 
	 * total, avg를 private으로 선언하면
	 * 누구도 직접 total, avg에 값을 저장할 수 없다. 
	 * 
	 * total, avg 변수에 담긴 값을 객체를 통해 읽기 위해 
	 * goetTotal(), getAvg() 메서드를 만들어 두었다.
	 * 
