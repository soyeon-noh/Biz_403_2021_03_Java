...
public class NumberServiceV1A {
	...
	/*
	 * method 이름은 addNum() 두 개의 정수를 매개변수로 받는다 누구나 어디선가 addNum()를 호출할 때는 객체.addNum()
	 * 2개의 정수를 전달하게 하겠다 라는 선언 객체.addNum(100,200) 두 개의 정수를 전달받기 위해서는 method가 준비를 해야
	 * 한다 method의 () 안에 변수를 선언한다 두개의 변수를 선언하자
	 * 
	 * 두 수의 합을 계산하여 결과가 짝수이면 그 합을 return, return 합; 형식의 명령이 있어야 한다. 그렇지 않으면 null을
	 * return 하라 return null; 형식의 명령이 있어야 한다.
	 * 
	 * return 값 형식의 명령이 있으려면 void 가 아닌 return type을 지정해 주어야 한다.
	 * 
	 * 단순히 정수만 return 하는 경우는 int type으로 하면되는데 여기서는 어떤 조건에 따라 null 값을 return 해야하기 떄문에
	 * Integer type으로 설정해야한다.
	 */
	 ...
	 * 		형식으로 method를 선언해야한다.
	 * 
	 * method를 호출할 때 전달하는 데이터의 type에 맞도록
	...
	public Float addNum(float num1, float num2) {
		...
		// method 코드가 아직 작성되지 않았을 때
		// 문법오류가 나는 것을 방지하는 임시코드
		return null;
	}
	
//	public Float addNum(float num2, float num2) {
//	
//} 이런건 안됨
