...
public class InputServiceV1A {
	...
	/*
	 * Scanner class 는 키보드 자원을 연동하여 키보드로부터 값을 입력받기 위한 클래스이다. 이 클래스는 컴퓨터의 자원을 사용하는
	 * 관리가 매우 엄격하게 필요한 클래스이다. 사용하는 방법에 따라, 오류가 발생하거나 심지어 컴퓨터 시스템이 멈추는 현상까지 발생할 수 있다.
	 * 이러한 클래스는 사용할 때 상당한 주의가 필요하다.
	 */

	// Scanner클래스를 main() method가 아닌
	// Service 등의 클래스에서 사용할 때는 클래스영역에서 선언하고,
	 ...
	 * 		정수 return되면 start, end 값 범위에 있는 값인지
	 * 			유효성 검사를 하고
	...
	public Integer inputValue(String title, int start, int end) {
		...
		// printf 같이 사용하는데 그 값을 출력하는 게 아니라 변수에 담는다.
		...
		while (true) {
			...
			if (retNum == null) {
				...
				System.out.printf("입력값은 %d ~ %d 범위내여야 합니다.\n"
						, start, end);
				continue;
			}
			return retNum;
	 ...
	 * 정수값이나 QUIT가 입력되는지 확인하고 
	 * 잘못 입력된 값이 있으면 다시 입력받도록 하는 method
	 */
	private Integer inputValue(String title) {
