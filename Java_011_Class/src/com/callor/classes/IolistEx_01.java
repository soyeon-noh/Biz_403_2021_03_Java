...
public class IolistEx_01 {
	 ...
	 * 			컴퓨터가 알 수 있는 부호들(코드)로
	 * 			변환하는 작업
	 * 		javac IolistEx_01.java //이클립스는 자동으로해줌.
	 * 3. 실행
	 ...
	 * 		IolistEx_01.main("어쩌고저쩌고") 를 호출하여
	 * 		코드를 실행하기 시작한다.
	 * 		
	 * 
	 * 		
	...
	public static void main(String[] arg) {
		 ...
		 * 3. 지금 만든 IolistVO클래스의 멤버변수는
		 * 		모두 private이기 때문에
		 * 		객체.멤버변수 형식으로는 값을 저장할 수 없다.
		 * 4. 멤버변수에 값을 저장할 때 setter method를
		 * 		통해서 저장해야 한다.
		 * 
		 * 
		 */
		IolistVOA ioVO = new IolistVOA();
		...
		printIolist(ioVO); //iolistVO 값이 printIolist의 매개변수로 복사가되는거기떄문에 꼭 이름이 같을 필요는 없다. ?? Q
		
	} // main()
	
	public static void printIolist(IolistVOA iolistVO) {
