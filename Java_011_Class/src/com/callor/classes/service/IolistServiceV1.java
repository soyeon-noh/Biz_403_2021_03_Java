 ...
 * 데이터의 묶음을 처리할 때는 보통 배열을 사용한다.
 * 하지만 배열은 그 구조상 사용이 다소 불편할 때가 많다.
 * Java에서는 배열을 사용해야 하는 프로젝트에서 
 * 		List 클래스를 사용한 객체를 생성하여 진행한다.
 * 
 */

public class IolistServiceV1 {
	/*
	 * 매입매출 데이터들을 사용하여 연산을 수행할텐데 매입매출 데이터들의 묶음을 List 형 객체 사용하겠다.
	 * 
	 * List형 객체에 포함된 (개별)데이터는 IolistVO 형으로 사용하겠다.
	 * 
	 * 낱개 데이터는 IolistVO 클래스 형으로 사용하고 그 데이터들을 묶어서 List 형으로 연산 수행을 하겠다는 선언.
	 */
	/*
	 * iolist를 private으로 선언한 이유 혹시 외부에서 iolist 데이터에 접근하여 값을 저장하는 일을 방지하기 위함이다.
	 * 
	 * V1 클래스를 상속하여 사용할 예정이라면 protected로 선언을 한다
	 */

	// TODO 멤버변수 선언하기
	protected List<IolistVOA> iolist;
	protected Scanner scan;

	public IolistServiceV1() {
		// TODO 클래스 생성자
		iolist = new ArrayList<IolistVOA>();
		scan = new Scanner(System.in);
	}
	
	// public 으로 선언된 method는
	// 		객체.method() 형식으로 호출이 가능하다.
	// return type이 명확히 결정되지 않았을 때는
	// 		void 형으로 작성한다.
	public void selectMenu() { // method 는 보통 public으로 선언
		// TODO 업무선택하기 메뉴
		while (true) {
			System.out.println("=".repeat(50)); //java 11부터 가능
		...
		// 만약 0번에 return을쓰면 여기가 아닌 if문에 써야함
		// 업무를 마쳤을때도 이 글이 나와야하기 때문에 이 방법이 더 나아보인다.
	}
	...
	public void printIolist(IolistVOA vo) {
		// TODO 1개 데이터 출력하기
